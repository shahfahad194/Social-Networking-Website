...
if(isset($_SESSION['fbuser'])){
		...
		$row_friend=$mysqli->query("select * from friends where user_id='$u_id' && status='Friend'");
		while($fetch_friend=$row_friend->fetch_array()){
		$u_id_f=$fetch_friend['user_id'];
		$user_friend_id	=$fetch_friend['user_friend_id'];
		
	$row3=$mysqli->query("select * from user_post where user_id='$user_friend_id' order by post_id desc");
		//$row3=$mysqli->query("select * from  user_post where user_id='$u_id' order by post_time desc");
				...
				";
					$result_random = rand(1,1000); 
					if($result_random<=100){
						echo"ad1";
						
					}elseif($result_random<=300){
						echo"ohk2";
						
					}elseif($result_random<=900){
						echo"ohk5";
			...
			<td style="border:solid lightgray 1px; padding-top:20px;">	
				...
				<?php echo"<table  id='po_all'  width='100%' height='auto' style='  margin-top:10px; font-family:sans-serif; text-transform:capitalize;'>
					...
					<?php 
							...
							echo"<td style='font-weight:bolder; color:gray;font-family:Algerian;'><a href='../Profile/Profile.php?id=$id' style='text-decoration:none;'>$name </a><br />$pt_time &nbsp<p title='Public' class='glyphicon glyphicon-globe'></p></td>
								<td></td>
								<td></td>
						...
						<?php
							...
							//for male and female pic post
								if (strchr($pt_pic,'mp4')=='mp4'){echo"<td style='padding:20px;'>$pt_txt</td></tr><tr><td style='padding:20px;' align='center'>";?><?php  if($gender=='Male' || $gender=='Female'){echo"  <video src='../../../fb_user/$gender/$email/post/$pt_pic'   controls id='vid' height='200px' width='auto' style=' z-index:1000; border:solid 1px lightgray; box-shadow:0px 0px 25px black;  '></video> </td> ";}}
								else{
								echo"<td style='padding:20px;'>$pt_txt</td></tr><tr><td style='padding:20px;'>";?><?php  if($gender=='Male' || $gender=='Female'){echo" <img src='../../../fb_user/$gender/$email/post/$pt_pic'   height='200px' width='100%' style='border:inset 1px lightgray; padding:0px; box-shadow:0px 0px 25px black; ' /></td> ";}}
								}else{
...
//$values = array($user_p_id=> array($post_id,$pt_txt));
//print_r($values);
	//$i=0;
  //foreach ($values as $key=>$user_p_id){
	 // echo $key;
		//foreach($user_p_id as $keys=>$post_id ){
		//	echo '<br/>'.$post_id;
		//}
  ...
  //}
//$row111=$mysqli->query("select * from users where user_id='$key'");
	//	$fetch111=$row111->fetch_array();
	//	$u_id_array=$fetch111['user_id'];
	//echo  $name_array=$fetch111['Name'];  
?> 
</td>
</tr>
</table>
